...
{
	int i;
	struct node** hashTable = (struct node**)calloc(TABLECAPA, sizeof(struct node*));
...
struct node* findNode(struct node*** hashTable, const char* key)
{
	...
	if(tmp->key == NULL && tmp->value == NULL && tmp->nextPtr == NULL)
	{
		return &emptyNode;
	}
	else
...
void insertNode(struct node*** hashTable, struct node* dataNode)
{
	...
	struct node** tmp = *hashTable+index;
	struct node* tmp2 = *(*hashTable+index);
...
char* itoa(int num)
{
	...
	div = 1;
	value = num;
	str = (char*)malloc(sizeof(char) * (int_length(num) + 1));
